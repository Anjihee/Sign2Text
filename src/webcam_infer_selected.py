...
INFERENCE_INTERVAL = 3.0  # 3초마다 예측
...
while cap.isOpened():
    ...
    # ✅ 상대 좌표 기반 feature 생성
    features = (
        extract_relative_coordinates(left_hand) +     # 42
        extract_relative_coordinates(right_hand) +    # 42
        calculate_angles(left_hand) +                 # 15
        calculate_angles(right_hand)                  # 15
    )

    if len(features) == 114:
        sequence.append(features)

        # ✅ 실시간 vs 학습 feature 비교
        ...
        print(f"❌ Feature length mismatch: {len(features)}")
